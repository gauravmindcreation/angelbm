...
abstract class Image_Driver
{
	...
	public function __construct($config)
	{
		if (is_array($config))
		{
			$this->config = array_merge(static::load_config(), $config);
		}
		else
		{
			$this->config = static::load_config();
		}
	...
	public function config($index = null, $value = null)
	{
		if (is_array($index))
		{
			...
			{
				throw new Exception("The driver cannot be changed after initialization!");
			}
	...
	public function preset($name)
	{
		...
		{
			throw new Exception("Could not load preset $name, you sure it exists?");
		}
		return $this;
	}
	...
	public function load($filename, $return_data = false)
	{
		...
		if (file_exists($filename))
		{
			...
			{
				throw new Exception("The library does not support this filetype for $filename.");
			}
		}
		else
		{
	...
	public function save($filename, $permissions = null)
	{
		...
		if ( ! touch($filename))
		{
			throw new Exception("Do not have permission to write to \"$filename\"");
		}

		// Set the new permissions
		if ($permissions != null and ! chmod($filename, $permissions))
		{
			throw new Exception("Could not set permissions on the file.");
		}
	...
	public static function load_config($key = null, $default = null)
	{
		$config = array(
			/**
