...
class Image_Gd extends \Image_Driver
{
	...
	public function load($filename, $return_data = false)
	{
		...
		{
			throw new Exception("Function imagecreatefrom".$image_extension."() does not exist (Missing GD?)");
		}
	...
	protected function _watermark($filename, $position, $padding = 5)
	{
		...
		if ($values == false)
		{
			throw new Exception("Watermark image not found or invalid filetype.");
		}
		else
		{
			extract($values);
