...
Joomla.submitbutton = function(task) {
	...
	if(task === 'success.cancel')
	{
		var confirmation = confirm('Have changes to save? Click Cancel, and then Save. Otherwise, click ok.');
	}
	if(confirmation)
	{
		if (task == 'success.cancel' || document.formvalidator.isValid(document.id('success-form'))) {
			Joomla.submitform(task, document.getElementById('success-form'));
		}
		else {
...
<form action="<?php echo JRoute::_('index.php?option=com_profiles&layout=edit&id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="success-form" class="form-validate" enctype="multipart/form-data">
	<div class="width-60 fltlft">
		...
		<fieldset class="adminform">
			<legend><?php echo JText::_('COM_PROFILES_TITLE_SUCCESSES'); ?></legend>
			<ul class="adminformlist">
				<li><?php echo $this->form->getInput('success_story'); ?></li>
				<li><?php
					echo $this->form->getLabel('success_story_image');
					echo $this->form->getInput('success_story_image');
					
					$img = '/uploads/recents/'.$id.'/165_120_'.$this->form->getValue('success_story_image');
					if(is_file(JPATH_SITE.$img))
					{
						$field = 'success_story_image';
						echo '
...
function delImage(photo_field)
{
	...
	{
		var xhr = new Request({
			url: 'index.php?option=com_profiles&tmpl=component&task=success.deletephoto&id=' + <?php echo $id; ?>,
			method: 'get',
			onSuccess: function()
			{
